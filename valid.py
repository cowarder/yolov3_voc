...
from utils import read_data_file, read_class_names, get_all_boxes, correct_yolo_boxes, nms, auto_thresh_nms
...
def main():
    ...
    weightfile = 'models_no_initial/epoch_199.weights'
    ...
    for i in r:
        ...
        weightfile = 'models_scratch/epoch_' + str(i) + '.weights'

        # valid(datafile, cfgfile, weightfile, prefix)

        valid_NB(datafile, cfgfile, weightfile, i, prefix)
        print(time.strftime('%Y.%m.%d %H:%M:%S', time.localtime(time.time())))
